...
- **Organic Polling Verification**: When testing thread-yielding functions like `Zawrs` spin-wait loops (`RiscVPause`), do not use boolean mock assertions (`EXPECT_TRUE(true)`). Prove execution organically by spawning a concurrent thread to set a flag, allowing the main test thread to repeatedly execute the yield loop. Assert that the loop increments and execution time scales correctly without CPU lockups.
