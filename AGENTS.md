...
- **C++ Include Ordering Guidelines**: Always group includes strictly following Google's C++ Style Guide: related headers first, followed by C system headers, C++ standard library headers, other libraries' headers, and finally project-specific headers, each separated by blank lines. Mixing C system headers (`<errno.h>`) with C++ standard headers (`<iostream>`) violates style boundaries.
