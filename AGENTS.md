...
### Testing Gotchas
- **Boot Protocol State Verification**: When writing bootloader constraints (e.g., `LinuxKernelBootloader`, `OpenSbiFirmwareLoader`), verify the processor state organically by executing a small assembly stub (`addi t0, a0, 0`) to confirm that `a0` and `a1` correctly propagated `hartid` and `.dtb` addresses, rather than solely checking the simulator's C++ proxy values.
