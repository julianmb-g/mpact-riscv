...
* **BootSequence Boundary Mismatch**: When creating a `vmlinux.elf` or similar boot payload, ensure that the `-Ttext` (or link script) address exactly matches the simulated entry point (e.g., `0x200000`). Mismatches between the compiler `-Ttext` boundary and the loader's expected PT_LOAD boundary cause the ELF loader to fail evaluation natively.
