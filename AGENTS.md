...
\n- **Profile Decoder Isolation:** When implementing privileged profiles (like RVA23S64) from unprivileged bases (like RVA23U64), a distinct `.isa`, `.bin_fmt`, and decoder cc_library target must be synthesized in the BUILD file to maintain structural separation, even if the current instruction subsets are largely identical.
