...
- **Fake "Happy-Path" Duplication Padding**: Tests that simply duplicate organic execution logic with different variables without adding structural coverage (e.g., `OpenSbiLinuxBootloaderTest` mirroring `TestLinuxBootProtocol`) must be deduplicated into parameterized tests to avoid bloating test execution time.
