...
[FLAG: stale] * **Watchdog Exception Integrity:** When implementing overarching C++ timeout exceptions natively, ensure that custom string headers (e.g., `trap_strings.h`) are correctly mapped into Bazel `deps` targets (e.g., `//sim:trap_strings`) to avoid compilation voids.
