...
#include "riscv/rva23s64_bin_decoder.h"
...
namespace mpact::sim::riscv::isa_rva23s64 {

using ::mpact::sim::generic::operator*;  // NOLINT: clang-tidy false positive.

Rva23s64Encoding::Rva23s64Encoding(RiscVState* state)
    : state_(state),
      inst_word_(0),
      opcode_(OpcodeEnum::kNone),
      format_(FormatEnum::kNone) {
  resource_pool_ = new generic::SimpleResourcePool("Rva23s64", 128);
...
Rva23s64Encoding::~Rva23s64Encoding() { delete resource_pool_; }

void Rva23s64Encoding::ParseInstruction(uint32_t inst_word) {
  inst_word_ = inst_word;
  if ((inst_word_ & 0x3) == 3) {
    auto [opcode, format] = DecodeRva23s64Inst32WithFormat(inst_word_);
    opcode_ = opcode;
    format_ = format;
    return;
  }
  auto [opcode, format] =
      DecodeRva23s64Inst16WithFormat(static_cast<uint16_t>(inst_word & 0xffff));
  opcode_ = opcode;
  format_ = format;
}

ResourceOperandInterface* Rva23s64Encoding::GetComplexResourceOperand(
...
ResourceOperandInterface* Rva23s64Encoding::GetSimpleResourceOperand(
...
DestinationOperandInterface* Rva23s64Encoding::GetDestination(
...
SourceOperandInterface* Rva23s64Encoding::GetSource(SlotEnum, int,
                                                         OpcodeEnum opcode,
