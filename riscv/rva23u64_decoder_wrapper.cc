...
Rva23u64DecoderWrapper::Rva23u64DecoderWrapper(RiscVState* state,
                                           util::MemoryInterface* memory)
    : state_(state), memory_(memory) {
  // Allocate the isa factory class, the top level isa decoder instance, and
  // the encoding parser.
  riscv_isa_factory_ = std::make_unique<RV64GVZBIsaFactory>();
  riscv_isa_ = std::make_unique<isa_rva23u64::Rva23u64InstructionSet>(
      state, riscv_isa_factory_.get());
  riscv_encoding_ = std::make_unique<isa_rva23u64::Rva23u64Encoding>(state);
  decoder_ = std::make_unique<RiscVGenericDecoder<
      RiscVState, isa_rva23u64::OpcodeEnum, isa_rva23u64::Rva23u64Encoding,
      isa_rva23u64::Rva23u64InstructionSet>>(
      state_, memory_, riscv_encoding_.get(), riscv_isa_.get());
}

generic::Instruction* Rva23u64DecoderWrapper::DecodeInstruction(
    uint64_t address) {
